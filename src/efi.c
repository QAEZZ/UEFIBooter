...
EFI_STATUS stats_for_nerds() {
  ...
  while (running) {
    ...
    con_output_stringf(ConOut,
                       ...
                       u"Columns: %d\r\n"
                       u"Rows: %d\r\n\r\n",
                       mode_info.MaxMode, mode_info.Mode, mode_info.Attribute,
                       mode_info.CursorColumn, mode_info.CursorRow,
                       mode_info.CursorVisible, dimensions.cols,
                       dimensions.rows);
    ...
    ConOut->SetCursorPosition(ConOut, dimensions.cols - sizeof(u"Hello mom!\r"),
                              dimensions.rows - 1);
...
char *strstr_ascii(char *haystack, char *needle) {
  if (!needle) {
    return haystack;
  }
...
BOOLEAN isdigit_ascii(char c) { return c >= '0' && c <= '9'; }

void fill_remaining_with_char(UINTN start_col, CHAR16 fill_char,
                              UINTN max_cols) {
  ConOut->SetCursorPosition(ConOut, start_col,
                            con_get_mode_info(ConOut).CursorRow);
...
EFI_STATUS read_esp_files(void) {
  ...
  status =
      BootServices->OpenProtocol(image, &lip_guid, (VOID **)&lip, image, NULL,
  ...
  status = BootServices->OpenProtocol(lip->DeviceHandle, &sfsp_guid,
                                      (VOID **)&sfsp, image, NULL,
                                      EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
  ...
  while (true) {
    ...
    ConOut->SetCursorPosition(ConOut, 0,
                              con_get_query_dimensions(ConOut).rows - 1);
    con_output_string(ConOut, u"Esc -> Go back\r");
    ...
    default:
      if (key.UnicodeChar == u'\r') {
        ...
        if (file_info.Attribute & EFI_FILE_DIRECTORY) {
          ...
          status = dirp->Open(dirp, &new_dir, file_info.FileName,
...
EFI_STATUS print_block_io_partitions(void) {
  ...
  status = BootServices->OpenProtocol(lip->DeviceHandle, &bio_guid,
                                      (VOID **)&biop, image, NULL,
                                      EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
  if (EFI_ERROR(status)) {
    con_output_stringf(ConOut,
  ...
  status = BootServices->LocateHandleBuffer(ByProtocol, &bio_guid, NULL,
  ...
  for (UINTN i = 0; i < num_handles; i++) {
    status = BootServices->OpenProtocol(handle_buffer[i], &bio_guid,
                                        (VOID **)&biop, image, NULL,
                                        EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
    if (EFI_ERROR(status)) {
      con_output_stringf(
    ...
    if (last_media_id != biop->Media->MediaId) {
      ...
      con_output_string_colored_latter(ConOut, u"Media ID: ",
                                       (last_media_id == this_image_media_id
                                            ? u"(Disk Image)\r\n"
                                            : last_media_id + u"\r\n"),
                                       EFI_LIGHTGRAY, DEFAULT_BG_COLOR);
    }
    ...
    con_output_stringf(ConOut,
                       ...
                       biop->Media->BlockSize, biop->Media->IoAlign,
                       biop->Media->LastBlock, biop->Media->LowestAlignedLba,
                       biop->Media->LogicalBlocksPerPhysicalBlock,
                       biop->Media->OptimalTransferLengthGranularity);

    if (!biop->Media->LogicalPartition) {
      con_output_string(ConOut, u"--<Entire Disk>\r");
      fill_remaining_with_char(strlen_u16(u"--<Entire Disk>"), u'-',
      ...
      status = BootServices->OpenProtocol(handle_buffer[i], &pi_guid,
                                          (VOID **)&pip, image, NULL,
                                          EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
      if (EFI_ERROR(status)) {
        con_output_stringf(ConOut,
        ...
        if (pip->Type == PARTITION_TYPE_MBR)
          ...
          if (pip->System == 1) {
            ...
            fill_remaining_with_char(strlen_u16(u"--<EFI Sys Partition>"), u'-',
                                     con_get_query_dimensions(ConOut).cols);
            con_output_string(ConOut, u"\r\n");
          } else {
            EFI_GUID bd_guid = BASIC_DATA_GUID;
            if (!memcmp(&pip->Info.Gpt.PartitionTypeGUID, &bd_guid,
                        sizeof(EFI_GUID))) {
              con_output_string(ConOut, u"--<Basic Data>\r");

              fill_remaining_with_char(strlen_u16(u"--<Basic Data>"), u'-',
                                       con_get_query_dimensions(ConOut).cols);
              con_output_string(ConOut, u"\r\n");
            } else {
              con_output_string(ConOut, u"--<Other GPT Type>\r");
              fill_remaining_with_char(strlen_u16(u"--<Other GPT Type>"), u'-',
                                       con_get_query_dimensions(ConOut).cols);
              con_output_string(ConOut, u"\r\n");
            }
          }
...
VOID *read_disk_lbas_to_buffer(EFI_LBA disk_lba, UINTN data_size,
  ...
  status = BootServices->OpenProtocol(handle_buffer[i], &dio_guid,
                                      (VOID **)&diop, image, NULL,
                                      EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
  if (EFI_ERROR(status)) {
    con_output_stringf(
  ...
  status = diop->ReadDisk(diop, disk_media_id,
                          disk_lba * biop->Media->BlockSize, data_size, buffer);
  if (EFI_ERROR(status)) {
    con_output_stringf(
...
EFI_STATUS get_disk_image_media_id(UINT32 *mediaID) {
  EFI_STATUS status = EFI_SUCCESS;
...
EFI_STATUS load_data_partition_file(char *file_to_load, CHAR16 *file_to_load_u16) {
  con_set_color(ConOut, DEFAULT_FG_COLOR, DEFAULT_BG_COLOR);
  con_clear_screen(ConOut);
  ...
  if (!file_buffer) {
    con_output_stringf(ConOut, u"Could not find nor read file %s to buffer\r\n",
                       datafls_info_path);
    goto exit;
  }
  ...
  if (!str_pos) {
    con_output_stringf(
        ConOut, u"Could not find file %s for file in data partition.\r\n",
        file_to_load);
    goto cleanup;
  }
  ...
  con_output_stringf(ConOut, u"File Size: %u bytes, Disk LBA: %u\r\n\r\n", file_size,
                     disk_lba);

  con_output_stringf(ConOut, u"Disk Contents for File %s:\r\n", file_to_load_u16);
...
EFI_STATUS load_boot_selector(void) {
  con_set_color(ConOut, DEFAULT_FG_COLOR, DEFAULT_BG_COLOR);
  con_clear_screen(ConOut);
  EFI_STATUS status = EFI_SUCCESS;
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
  ...
  const EFI_STATUS (*menu_funcs[])(void) = {
      stats_for_nerds,
      // system_shutdown,
      read_esp_files, print_block_io_partitions, load_boot_selector};
